...
export function CheckoutModal({ open, items, subtotal, discount, couponCode, deliveryZone, notas, products, onZoneChange, onClose, onBack, onSuccess, onAddProduct, profile }: P) {
  ...
  const suggested = useMemo(() => products.filter(p => p.is_available && !cartIds.has(p.id)).slice(0, 6), [products, cartIds]);
  ...
  const renderMerchant = (mid: string) => {
    ...
    switch (mid) {
      case 'pago_movil': return rows([{ icon: Landmark, label: 'Banco', val: BANK_DATA.bank }, { icon: Hash, label: 'RIF', val: BANK_DATA.rif }, { icon: Phone, label: 'Tel', val: BANK_DATA.phone }, { icon: Hash, label: 'Cuenta', val: BANK_DATA.account }]);
      case 'transferencia': return rows([{ icon: Landmark, label: 'Banco', val: TRANSFER_DATA.bank }, { icon: Hash, label: 'RIF', val: TRANSFER_DATA.rif }, { icon: Hash, label: 'Cuenta', val: TRANSFER_DATA.account }, { icon: User, label: 'Titular', val: TRANSFER_DATA.holder }]);
      case 'binance': return rows([{ icon: Hash, label: 'Pay ID', val: BINANCE_DATA.payId }, { icon: Landmark, label: 'Red', val: BINANCE_DATA.network }, { icon: User, label: 'Titular', val: BINANCE_DATA.holder }]);
      case 'paypal': return rows([{ icon: Phone, label: 'Email', val: PAYPAL_DATA.email }, { icon: User, label: 'Titular', val: PAYPAL_DATA.holder }]);
      case 'tarjeta_internacional': return <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-3"><Landmark className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" /><div><p className="text-sm font-bold text-blue-800">{CARD_DATA.processor}</p><p className="text-xs text-blue-600 mt-0.5">{CARD_DATA.note}</p></div></div>;
      default: return null;
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      ...
      {status === 'success' ? <div className="flex-1 flex flex-col items-center justify-center px-8 text-center"><div className="w-20 h-20 rounded-full bg-green-50 flex items-center justify-center mb-5 animate-bounce"><Check className="w-10 h-10 text-green-500" strokeWidth={3} /></div><h3 className="text-xl font-bold text-gray-900 mb-2">¡Pedido confirmado!</h3></div>
      : <div className="flex-1 overflow-y-auto"><form onSubmit={submit} className="px-5 py-4 space-y-5">
        ...
        <div className="space-y-3"><h3 className="text-sm font-bold text-gray-900 uppercase flex items-center gap-2"><CalendarClock className="w-4 h-4 text-orange-400" /> Entrega</h3><div className="flex gap-2">{(['inmediata', 'programada'] as const).map(t => <button key={t} type="button" onClick={() => setSchedType(t)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${schedType === t ? 'bg-orange-500 text-white border-orange-500' : 'bg-gray-50 text-gray-500 border-gray-100'}`}>{t === 'inmediata' ? 'Inmediata' : 'Programar'}</button>)}</div>{schedType === 'programada' && <div className="flex gap-2"><input type="date" required value={schedDate} min={new Date().toISOString().split('T')[0]} onChange={e => setSchedDate(e.target.value)} className="flex-1 py-2.5 px-3 rounded-xl bg-gray-50 border border-gray-100 text-sm focus:outline-none focus:border-orange-300" /><input type="time" required value={schedTime} onChange={e => setSchedTime(e.target.value)} className="flex-1 py-2.5 px-3 rounded-xl bg-gray-50 border border-gray-100 text-sm focus:outline-none focus:border-orange-300" /></div>}</div>
        <div className="space-y-3"><h3 className="text-sm font-bold text-gray-900 uppercase">Pago</h3>{PAYMENT_METHODS.map(({ id, label, sub, Icon }) => <label key={id} className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all active:scale-[0.98] ${form.pm === id ? 'border-orange-500 bg-orange-50/50' : 'border-gray-100 bg-gray-50'}`}><div className={`w-10 h-10 rounded-lg flex items-center justify-center ${form.pm === id ? 'bg-orange-500' : 'bg-gray-100'}`}><Icon className={`w-5 h-5 ${form.pm === id ? 'text-white' : 'text-gray-400'}`} /></div><div className="flex-1"><p className="font-bold text-sm">{label}</p><p className="text-xs text-gray-400">{sub}</p></div><div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${form.pm === id ? 'border-orange-500 bg-orange-500' : 'border-gray-200'}`}>{form.pm === id && <Check className="w-3 h-3 text-white" strokeWidth={3} />}</div><input type="radio" name="pm" value={id} checked={form.pm === id} onChange={e => setForm({ ...form, pm: e.target.value })} className="sr-only" /></label>)}{am && <div className="space-y-3 animate-slide-up">{renderMerchant(am.id)}{am.fields.map(fld => <div key={fld.key}><label className="text-xs font-semibold text-gray-400 mb-1.5 block">{fld.label} {fld.required && '*'}</label><div className="relative"><Hash className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-300" /><input type="text" value={gfv(am.id, fld.key)} onChange={e => { let v = e.target.value; if (fld.numericOnly) v = v.replace(/\D/g, ''); if (fld.maxLength) v = v.slice(0, fld.maxLength); sfv(am.id, fld.key, v); }} placeholder={fld.placeholder} maxLength={fld.maxLength} className={`w-full pl-10 pr-4 py-3 rounded-xl bg-gray-50 border border-gray-100 text-sm focus:outline-none focus:border-orange-300 focus:bg-white ${fld.numericOnly ? 'font-mono tracking-widest' : ''}`} /></div></div>)}{am.currency === 'BSV' && <div>{ss ? <div className="relative rounded-xl overflow-hidden border border-gray-100"><img src={ss} alt="Captura" className="w-full h-40 object-cover" /><button type="button" onClick={() => setSs(null)} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/60 flex items-center justify-center"><X className="w-4 h-4 text-white" /></button></div> : <label className="flex flex-col items-center justify-center gap-2 py-6 rounded-xl bg-gray-50 border-2 border-dashed border-gray-200 cursor-pointer hover:border-orange-300 active:scale-[0.98]"><ImagePlus className="w-7 h-7 text-gray-300" /><span className="text-xs text-gray-400">Adjuntar captura</span><input type="file" accept="image/*" onChange={handleFile} className="sr-only" /></label>}</div>}</div>}</div>
